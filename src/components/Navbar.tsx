    ...
    <nav className="fixed top-0 left-0 right-0 z-50 glass">
      <div className="container mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          {user && <Link to="/upload" className="text-sm text-muted-foreground hover:text-foreground transition-colors">Subir</Link>}
        </div>
          ...
          {user ? (
            <>
              <Link to="/profile" className="flex items-center gap-2 px-4 py-2 rounded-full border border-border hover:bg-secondary transition-colors">
                ...
                <span className="text-sm font-display font-medium text-foreground truncate max-w-[100px]">
      ...
      {isOpen && (
        <div className="md:hidden px-6 pb-4 flex flex-col gap-3">
          <Link to="/" className="text-sm py-2 text-muted-foreground" onClick={() => setIsOpen(false)}>Inicio</Link>
          <Link to="/explore" className="text-sm py-2 text-muted-foreground" onClick={() => setIsOpen(false)}>Explorar</Link>
          <Link to="/recommendations" className="text-sm py-2 text-muted-foreground" onClick={() => setIsOpen(false)}>Para Ti</Link>
          ...
          {user ? (
            ...
            <>
              <Link to="/login" className="text-sm py-2 text-muted-foreground" onClick={() => setIsOpen(false)}>Iniciar Sesión</Link>
              <Link to="/register" className="px-5 py-2 rounded-full bg-primary text-primary-foreground font-display font-semibold text-sm text-center" onClick={() => setIsOpen(false)}>
                Registrarse
              </Link>
            </>
          )}
